...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if(config.poolServer.type == "pps"){
        redisCommands.push(['hincrbyfloat', config.coin + ':shares:good', miner.login, (job.difficulty/shareDiff)]);
        redisCommands.push(['hincrbyfloat', config.coin + ':shares:good:total', miner.login, (job.difficulty/shareDiff)]);
    }
