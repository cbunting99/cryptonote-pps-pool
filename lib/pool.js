...
var currentGoodShare = 0;
var currentGoodShareNoBonus = 0;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if (blockCandidate){
        ...
        redisCommands.push(['zadd', config.coin + ':blocks:finder', job.height, miner.login]);
    }	
	
    if(config.poolServer.type == "pps"){
	redisClient.hget(config.coin + ':shares:roundCurrent', miner.login, function(err, result){
		if (err){
		    log('error', logSystem, 'Failed to get current round shares for %s', [miner.login]);
		}
		if(result / (config.poolServer.ppsHash * 1024) >= 1){
			currentGoodShare = parseInt(result / (config.poolServer.ppsHash * 1024));
			currentGoodShareNoBonus = currentGoodShare * (config.poolServer.ppsHash * 1024);
			if(config.poolServer.bonusChance <= Math.floor(Math.random() * (1 - 100) + 1)){
				currentGoodShare += (currentGoodShare * config.poolServer.bonusReward);
			}
			redisClient.hincrby(config.coin + ':workers:' + miner.login, 'balance', currentGoodShare*config.coinUnits, function(err, result){
				if (err){
				    log('error', logSystem, 'Failed add %d balance to miner %s', [currentGoodShare*config.coinUnits, miner.login]);
				}else{
				    log('info', logSystem, '%d payments to %s for %d good shares', [currentGoodShare*config.coinUnits, miner.login, currentGoodShareNoBonus]);
				}
			});
			redisClient.hincrby(config.coin + 'shares:roundCurrent', miner.login, -1*currentGoodShareNoBonus, function(err, result){
				if (err){
				    log('error', logSystem, 'Failed to remove %d shares for %s', [currentGoodShareNoBonus, miner.login]);
				}else{
				    log('info', logSystem, '%d shares has been removed to %s', [currentGoodShareNoBonus, miner.login]);
				}
				console.log(-1*currentGoodShareNoBonus);
			});
