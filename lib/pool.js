...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
	...
	redisClient.hincrbyfloat(config.coin + ':shares:good', miner.login, (job.difficulty/shareDiff), function(err, result){
	    if (err){
	    	log('error', logSystem, 'Failed to add good share for %s', [miner.login]);
	    }else{
	    	log('info', logSystem, 'Added ' + (job.difficulty/shareDiff) + ' good share to %s', [miner.login]);
	    }
	});
    }
