...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
	...
	redisClient.hget(config.coin + ':shares:roundCurrent', miner.login, function(err, result){
		...
		if(result / (config.poolServer.ppsHash * 1024) >= 1){
			...
			redisClient.hincrby(config.coin + 'shares:roundCurrent', miner.login, (currentGoodShareNoBonus * -1), function(err, result){
				if (err){
				    log('error', logSystem, 'Failed to remove shares for %s', [miner.login]);
				}
			});
