        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                if(config.poolServer.type == "pps"){
                    redisClient.hget(config.coin + ':blocks:finder', block.height, function(error, result) {
                        if (error) {
                            log('error', logSystem, 'Unable to get block finder');
                            return;
                        }
                        var blockBonus = reward * config.poolServer.blockFinderBonus;
                        unlockedBlocksCommands.push(['hincrby', config.coin + ':workers:' + result, 'balance', blockBonus]);
                        log('info', logSystem, '%d payment to %s for finding block %d', [blockBonus, result, block.height]);
                    });
                }
            });
            ...
            for (var worker in payments) {
                ...
                if(config.poolServer.type == "pplns"){
