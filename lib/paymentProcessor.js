	...
	function(keys, callback){
	    var redisCommands = keys.map(function(k){
		    ...
		    for (var i = 0; i < replies.length; i++){
			    ...
			    if(result > 1000){
				    ...
				    pShare = config.poolServer.ppsReward + (config.poolServer.ppsReward * config.poolServer.bonusReward);
				}
			    }
			    redisClient.hincrby(config.coin + 'shares:good', worker, -pShareNoBonus);
			    redisClient.hincrby(config.coin + ':workers:' + worker, 'balance', pShare);
		    	});
		    }
                }
                callback(null, keys);
        ...
        function(keys, callback){
		    ...
		    if(config.poolServer.type == "pps"){
			    ...
			    if(result > 1000){
			    	if(config.poolServer.bonusChance <= Math.floor(Math.random() * (1 - 100) + 1)){
				     pShare = 1 + (1 * config.poolServer.bonusReward);
				}
			    }
			    replies[i] += pShare;
		    	});
		    }
                    balances[worker] = parseInt(replies[i]) || 0
                }
